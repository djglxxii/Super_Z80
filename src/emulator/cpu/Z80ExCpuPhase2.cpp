...
Z80ExCpuPhase2::Z80ExCpuPhase2(Phase2Bus& bus) : bus_(bus) {
  cpu_ = z80ex_create(
      Z80ExPhase2Callbacks::MemRead, this,
      Z80ExPhase2Callbacks::MemWrite, this,
      Z80ExPhase2Callbacks::PortRead, this,
      Z80ExPhase2Callbacks::PortWrite, this,
      Z80ExPhase2Callbacks::IntRead, this);
  SZ_ASSERT(cpu_ != nullptr);
  Reset();
}

Z80ExCpuPhase2::~Z80ExCpuPhase2() {
  if (cpu_ != nullptr) {
...
uint32_t Z80ExCpuPhase2::RunTStates(uint32_t tstate_budget) {
  SZ_ASSERT(cpu_ != nullptr);
  if (tstate_budget == 0) {
    return 0;
  }
...
void Z80ExCpuPhase2::SetIntLine(bool asserted) {
  int_line_ = asserted;
}

Z80DebugState Z80ExCpuPhase2::GetDebugState() const {
  return dbg_;
}

void Z80ExCpuPhase2::RefreshDebugRegs() {
  SZ_ASSERT(cpu_ != nullptr);
